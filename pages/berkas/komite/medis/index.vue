...
useHead({
  title: `Berkas komite ${lastSegment} -- ILM `,
...
async function doDelete() {
  ...
  const { data: postData, error, refresh, status } = await useFetch(`${API_V2_URL}/berkas/komite/${lastSegment}/${btoa(`${rowSelected.value.nomor}.${rowSelected.value.tgl_terbit}`)}`, {
    params: { _method: 'PUT' },
    method: "POST",
