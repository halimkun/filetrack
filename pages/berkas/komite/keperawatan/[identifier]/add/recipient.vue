...
const onSubmit = async () => {
  ...
  if (berkasKomite?.value?.data?.no_surat) {
    let body = {
      ...
      tipe: `komite/${komite}`,
      model: runtimeConfig.public.modelName.KOMITE_KEPERAWATAN,
    ...
    if (error.value) {
      logEvent('post_berkas_komite_failed', { error: error.value, komite: komite, identifier: route.params.identifier })
      console.error('Error fetching surat internal:', error.value);
    }

    if (status.value == 'success') {
      logEvent('post_berkas_komite_success', { data: data })
      toast.add({
        title: 'Berhasil',
